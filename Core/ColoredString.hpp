...
public:
	...
	constexpr ColoredChar(
		const char chr,
		const OS::ColorType color = OS::Color::None
	) noexcept;
...
{
private:
	// Storing the color of each char requires a simpler implementation and gives better performance,
	// but it takes more memory than a color-change set implementation. The former is preferred since
	// ColoredString's aren't usually going to be stored for long.
