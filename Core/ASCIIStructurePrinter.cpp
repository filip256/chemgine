...
const BondedAtomBase& Node::getAtom() const
{
    return *atom;
}

const ColoredString& Node::getSymbol() const
{
    return symbol;
}

Symbol::SizeT Node::getSymbolSize() const
{
    return static_cast<Symbol::SizeT>(symbol.size());
}
...
PositionLine Node::getPosition() const
{
    return HLine(position, getSymbolSize());
}

bool Node::visited() const
{
    return not utils::isNPos(position);
}

void Node::unvisit()
{
    position = utils::npos<Position>;
}
...
void Node::addContainingCycle(Cycle& cycle)
{
    containingCycles.emplace_back(&cycle);
}
...
OS::ColorType Edge::getColor() const
{
    return color;
}

const Bond& Edge::getBond() const
{
    return *bond;
}
...
const std::vector<const BondedAtomBase*>& Cycle::getCycle() const
{
    return cycle;
}

const BondedAtomBase& Cycle::getAtom(const c_size idx) const
{
    return *cycle[idx];
}

c_size Cycle::size() const
{
    return static_cast<c_size>(cycle.size());
}

bool Cycle::visited() const
{
    return not utils::isNPos(centroid);
}

void Cycle::unvisit()
{
    centroid = utils::npos<Point<float>>;
}
...
{
    return static_cast<c_size>(std::ranges::count_if(cycle, [&](const auto* atom)
        {
            return nodes[atom->index].visited();
        }));
}
...
public:
    ...
    {
        return scoreSum / positions.size();
    }
...
private:
    ...
    {
        return std::any_of(newPositions.begin(), newPositions.end(), [position](const auto pos) { return position.intersects(pos); });
    }

    static bool areOverlapping(const PositionLine& positionA, const PositionLine& positionB, const std::vector<PositionLine>& newPositions)
    {
        ...
        // Edges aren't stored in the state so their position must be inferred from the node positions.
        const auto edgePos = StructurePrinter::getNextEdgePosition(positionA, Direction(positionB.origin - positionA.origin));
...
public:
    ...
    bool arePositionsTaken(const PositionLine& firstPosition, const std::vector<PositionLine>& newPositions) const
    {
        ...
        // There is a generated edge between the first (fixed) position and second generated position but
        // the fixed position is not stored in the state to save memory.
...
std::vector<std::pair<ASCII::Direction, Position>> StructurePrinter::getPossibleNextDirections(
...
void StructurePrinter::print()
{
    ...
    // Cyclic:
    for (auto& c : cycles)
        for (auto* a : c.getCycle())
            nodes[a->index].addContainingCycle(c);

    const auto largestCycleIt = std::ranges::max_element(cycles, [](const auto& lhs, const auto& rhs)
        {
...
ColoredTextBlock& StructurePrinter::getBlock()
{
    return buffer;
}

ColoredString StructurePrinter::toString() const
{
...
std::optional<std::pair<Position, Direction>> StructurePrinter::inferEdgePosition(
...
std::optional<std::pair<Position, Direction>> StructurePrinter::inferEdgePosition(
    const PositionLine from, const PositionLine to)
{
...
bool StructurePrinter::isAmbiguousBondPlacement(const Position position, const Direction direction, const BondType bondType,
...
std::tuple<Position, PositionLine, ASCII::Direction> StructurePrinter::getFreeCycleClosurePosition(
    const PositionLine prevNodePos, const BondType bondType,
    const Symbol::SizeT closureSymbolSize, const std::array<Direction, 8>& directions) const
{
    for (const auto dir : directions)
    {
        const auto [edgePos, closurePos] = getNextPosition(prevNodePos, dir, closureSymbolSize);
        if (buffer.isWhiteSpace(edgePos) && buffer.isWhiteSpace(closurePos) &&
...
std::tuple<Position, PositionLine, ASCII::Direction> StructurePrinter::getFreeCycleClosurePositionLookahead(
    const PositionLine prevNodePos, const BondType bondType, const Symbol::SizeT closureSymbolSize,
    const Position prevBondPos, const Direction prevBondDirection, const BondType prevBondType,
    ...
    const Direction enteringDirection) const
{
    ...
    const auto [firstPrevDirection, lastDirection] = noConstraints ?
        utils::npos<std::pair<Direction, Direction>> :
        std::make_pair(
            Direction(firstPosition.origin - nodes[cycle[(firstCycleIdx + cycle.size() - 1) % cycle.size()]->index].position),
    ...
    if (totalAtomsToPrint == 2 && not noConstraints)
    {
        ...
        for (uint8_t dirIdx = 0; dirIdx < directions.size(); ++dirIdx)
        {
            ...
            if (not buffer.isWhiteSpace(lastEdgePos) || isAmbiguousBondPlacement(lastEdgePos, dirToFirst, lastToFirstBond.getType()))
            ...
            const auto score =
                (combineScores(firstPrevDirection.getCyclicAngleScore(dir), firstToSecondBond.getASCIIScore(dir)) * directionOrderModifier +
                    combineScores(dir.getCyclicAngleScore(dirToFirst), lastToFirstBond.getASCIIScore(dirToFirst)) +
                    combineScores(dirToFirst.getCyclicAngleScore(lastDirection), 1.0f)) / 3.0f;
    ...
    else
    {
        ...
        for (uint8_t dirIdx = 0; dirIdx < directions.size(); ++dirIdx)
        {
            ...
            // The first directions have slightly higher score.
            const auto directionOrderModifier = dirOrderModifierQuant(dirIdx);
            const auto initialScore = combineScores(noConstraints ? 1.0f : firstPrevDirection.getCyclicAngleScore(dir), firstToSecondBond.getASCIIScore(dir)) * directionOrderModifier;
    ...
    while (stack.size())
    {
        ...
        {
            Log(this).trace("Generated optimal layout of size {0} (score: {1}, stack size: {2}).", totalAtomsToPrint - (not noConstraints), current.getScore(), stack.size() + 1);
            return std::move(current.positions);
        }

        const auto firstDirection = Direction(current.positions.front().origin - firstPosition.origin);
        const auto prevDirection = positionCount >= 2 ?
            Direction(current.positions.back().origin - current.positions[positionCount - 2].origin) :
            firstDirection;
        const auto prevAngle =
            positionCount >= 3 ? Direction(current.positions[positionCount - 2].origin - current.positions[positionCount - 3].origin).getAngle(prevDirection) :
            ...
            for (const auto& dirToLast : Direction::AllDirections)
            {
                ...
                auto newScoreSum = noConstraints ?
                    (current.scoreSum
                        ...
                        + combineScores(prevDirection.getCyclicAngleScore(dirToLast), currentToLastBond.getASCIIScore(dirToLast))
                        + combineScores(dirToLast.getCyclicAngleScore(dirToFirst), lastToFirstBond.getASCIIScore(dirToFirst))
                        + combineScores(dirToFirst.getCyclicAngleScore(firstDirection), firstToSecondBond.getASCIIScore(firstDirection)))
                    * newPositions.size() / (newPositions.size() + 1) :
                    (current.scoreSum
                        + combineScores(prevDirection.getCyclicAngleScore(dirToLast), currentToLastBond.getASCIIScore(dirToLast))
                        + combineScores(dirToLast.getCyclicAngleScore(dirToFirst), lastToFirstBond.getASCIIScore(dirToFirst))
                        + combineScores(dirToFirst.getCyclicAngleScore(lastDirection), 1.0f))
                    * newPositions.size() / (newPositions.size() + 2);
        ...
        for (const auto& dir : Direction::AllDirections)
        {
            ...
            const auto remainingAtoms = currentIdx < lastCycleIdx ?
                lastCycleIdx - currentIdx - 2 :               // current->end
    ...
    const c_size lastCycleIdx)
{
    ...
    const auto linearExpand = [&](const c_size endIdx, const bool backward)
        {
            ...
            std::ranges::sort(directionsCopy, [&](const auto lhs, const auto rhs)
                {
                    const auto lhsScore = utils::isqrt(round_cast<uint16_t>(printAwayPoint.squaredDistance(prevPosition.origin + lhs.get() * scalingFactor)));
                    const auto rhsScore = utils::isqrt(round_cast<uint16_t>(printAwayPoint.squaredDistance(prevPosition.origin + rhs.get() * scalingFactor)));
                    return lhsScore > rhsScore;
                });

            auto tempClosurePos = getFreeCycleClosurePosition(prevPosition, bondToNext->getType(),
                static_cast<Symbol::SizeT>(closureSymbol.size()), directionsCopy);
            ...
            while (idx != endIdx)
            {
                ...
                for (const auto dir : directionsCopy)
                {
                    ...
                    // The lookahead also checks that the bond to current isn't ambiguous, so no need to check above.
                    const auto bondToClosure = currentAtom->getBondTo(cycle.getAtom(nextIdx));
                    tempClosurePos = getFreeCycleClosurePositionLookahead(
                        nodePos, bondToClosure->getType(), static_cast<Symbol::SizeT>(closureSymbol.size()),
                        edgePos, dir, bondToNext->getType(), directionsCopy);
                    if (utils::isNPos(tempClosurePos))
                        continue;
    ...
    for (const auto& [node, prevDir] : printedAtoms)
        printNeighbors(*node, prevDir, &cycle);
}
...
void StructurePrinter::printImpliedHydrogens(
    const Node& node)
{
...
void StructurePrinter::printEdge(const Position position, const Direction direction, const BondedAtomBase& from, const BondedAtomBase& to)
{
...
void StructurePrinter::printUnmaterializedEdges()
{
    ...
    for (const auto& [indexes, edge] : edges)
    {
        ...
        {
            const auto [edgePos, closurePos, dir] =
                getFreeCycleClosurePosition(nodeA.getPosition(), edge.getBond().getType(),
                    static_cast<Symbol::SizeT>(closureSymbol.size()), getLinearPreferredDirections());
    ...
    const Cycle* cycle)
{
    ...
    public:
        ...
        {
            return bond->getOther();
        }

        static Neighbor create(
            const Bond& bond,
            std::vector<Node>& nodes)
        {
    ...
    for (const auto& n : neighbors)
    {
        ...
        else if (cycle != nullptr)
        {
            ...
            std::ranges::sort(directionsCopy, [&](const auto lhs, const auto rhs)
                {
                    const auto lhsScore = utils::isqrt(round_cast<uint16_t>(printAwayPoint.squaredDistance(node.position + lhs.get() * scalingFactor)))
                        * bond.getASCIIScore(lhs);
                    const auto rhsScore = utils::isqrt(round_cast<uint16_t>(printAwayPoint.squaredDistance(node.position + rhs.get() * scalingFactor)))
        ...
        for (uint8_t dirIdx = 0; dirIdx < directionsCopy.size(); ++dirIdx)
        {
            ...
            if (n.cycle != nullptr)
            {
                ...
                {
                    // If there are no existing constraints, the first atom is placed as if it were non-cyclic, forming an initial
                    // constraint for the layout generator.
            ...
            else
            {
                ...
                // The propagation change is only applied if the best-score direction was used for printing.
...
void StructurePrinter::printCycle(
    const c_size startCycleIdx,
    Cycle& cycle,
    const Direction enteringDirection)
{
    // If another atom of the cycle has already been printed, it's position is considered a constraint.
    // This should only occur if this cycle is part of a poly-cycle and at least one of its adjacent cycles has already been printed.
    // Because we use the minimal cycle basis and we print cycles one-by-one the constraints should always be a single contiguous
    // sequence nodes containing the starting atom.

    // If the states of the stack are ordered by a score (priority_queue) then the first layout which matches all requirements
    // should also be the one with the highest possible score.
    // When such a layout is found it is copied to the original buffer and the printing recursion continues for all non-visited
    // members of the cycle.
    // If no suitable layout is found the cycle is printed linearly starting from the first atom and cycle closures are added
    ...
    {
        // Case 1: Constraint sequence ends in C. L iterates backwards from C until the first unvisited atom, S=C, F=S-1.
        // ..-L-C..C-F-S-..
        do
        {
    ...
    {
        // Case 2: Constraint sequence starts with C. S iterates forward from C until the first unvisited atom, F=S-1, E=C+1.
        // ..-S-F-C..C-L-.. -> ..-L-C..C-F-S-..
        do
        {
    ...
    if (positions.empty() && totalAtomsToPrint <= 18)
        positions = generateOptimalCycleLayout(cycle.getCycle(), firstCycleIdx, secondCycleIdx, lastCycleIdx, noConstraints, enteringDirection);
    if (positions.empty())
    {
    ...
    private:
        Member(
            ...
            const c_size unprintedNeighbourCount
        ) noexcept :
            atom(&atom),
    ...
    public:
            ...
            const std::vector<Node>& nodes)
        {
            ...
            const auto unprintedNeighbourCount = static_cast<c_size>(std::ranges::count_if(atom.bonds, [&](const auto& bond)
                {
    ...
    if (options.has(PrintFlags::PRINT_CYCLE_ORDER) && cycle.size() >= 4)
    {
        ...
        const ColoredString label(std::to_string(std::ranges::count_if(cycles, [](const auto& c) { return c.visited(); })), OS::Color::DarkGrey);
        if (buffer.isWhiteSpace(centerPos, label.size()))
            buffer.insert(centerPos, label);
    }

    // Populate printed nodes (must be done after the center is computed and the cycle is marked as visited).
    ...
    std::ranges::sort(printedAtoms, [&](const auto& lhs, const auto& rhs)
        {
            return
                lhs.totalConstraintCount > rhs.totalConstraintCount ? true :
