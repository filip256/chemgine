...
bool DefUnitTest::run()
{
	...
	if (not dataStore.addDefinition(std::move(*def)))
	{
...
bool DefLoadUnitTest::run()
{
	...
	{
		Log(this).error("Store contains {0} unused estimators after load completion.",
...
DefClearUnitTest::DefClearUnitTest(
	std::string&& name,
	DataStore& dataStore
) noexcept:
	UnitTest(std::move(name)),
	...
	UnitTestGroup(std::move(name), filter)
{
	...
	registerTest<DefUnitTest>("radical",
		"_radical:* {"
			"name :any radical,"
			"matches:*"
		"};");

	registerTest<DefUnitTest>("data_ool",
		"_data < d49 > : ( K ,torr  ) -> J/mol  {"
			"mode: LINEAR, values: { { 0.0, 1.0 } : 40700.0, { 1.0, 1.0 } : 40900.0 }"
		"};");

	registerTest<DefUnitTest>("mol",
		"_mol: O {"
			"name:            water,"
	...
	registerTest<DefUnitTest>("react",
	...
	registerTest<DefUnitTest>("react_cut",
	...
	registerTest<DefUnitTest>("lab_condenser",
		"_lab: condenser{"
			...
			"tx : ./Textures/liebig300.png,"
			"inner_mask : ./Textures/liebig300_if.png,"
			"coolant_mask : ./Textures/liebig300_cf.png,"
		"};");
	...
	registerTest<DefDumpUnitTest>("dump", dataStore, "./TestFiles/temp0.cdef", false);
	registerTest<DefClearUnitTest>("clear", dataStore);
	registerTest<DefLoadUnitTest>("load", dataStore, "./TestFiles/temp0.cdef", true);
	registerTest<DefCountUnitTest>("count", dataStore, 213);
	registerTest<DefDumpUnitTest>("dump_pretty", dataStore, "./TestFiles/temp1.cdef", true);
	registerTest<DefLoadUnitTest>("reload", dataStore, "./TestFiles/temp1.cdef", false);
	registerTest<DefCountUnitTest>("count", dataStore, 213);
	registerTest<DefClearUnitTest> ("clear", dataStore);
	registerTest<DefLoadUnitTest>("load_pretty", dataStore, "./TestFiles/temp1.cdef", true);
	registerTest<DefCountUnitTest>("count", dataStore, 213);
	registerTest<DefClearUnitTest>("clear", dataStore);
