	...
	const uint16_t baseIndent) const
{
	...
	if (prettify)
	{
		static const uint8_t valueOffset = Utils::max(
			Def::Data::Mode.size(),
			Def::Data::Parameters.size()) + 2;

		out << ": " << UnitizedEstimator<OutU, InUs...>::getUnitSpecifier(true);
		out << " {\n";
		...
		out << indentStr << Def::Syntax::Indent << Def::Data::Parameters << ':' << std::string(valueOffset - Def::Data::Parameters.size(), ' ')
			<< Def::prettyPrint(regressor.getParams());
		out << '\n' << indentStr << '}';
	...
	{
		out << ':' << UnitizedEstimator<OutU, InUs...>::getUnitSpecifier(false);

		out << '{';
		out << Def::Data::Mode << ':' << Def::print(getMode()) << ',';
		out << Def::Data::Parameters << ':' << Def::print(regressor.getParams());
		out << '}';
