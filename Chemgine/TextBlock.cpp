...
TextBlock::TextBlock(const size_t sizeX, const size_t sizeY) noexcept
{
	block.reserve(sizeY);
	expandTo(sizeX, sizeY);
}

void TextBlock::expandTo(const size_t sizeY)
{
	for (size_t i = block.size(); i < sizeY; ++i)
		block.emplace_back(1);
}

void TextBlock::expandTo(const size_t sizeX, const size_t sizeY)
{
	for (size_t i = 0; i < block.size(); ++i)
		block[i].expandTo(sizeX);
	for (size_t i = block.size(); i < sizeY; ++i)
		block.emplace_back(sizeX);
}
...
void TextBlock::trim()
{
	...
	size_t idx = 0;
	while (idx < block.size() && block[idx].isWhiteSpace()) ++idx;
	hStart = idx;
