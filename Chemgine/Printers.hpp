...
namespace Def
{
	...
	template <typename T>
	static std::string printId(const T object);
	...
	public:
		...
		{
			return 
				Utils::equal(object, std::numeric_limits<T>::lowest()) ? Def::Amounts::Min :
				Utils::equal(object, std::numeric_limits<T>::max()) ? Def::Amounts::Max :
...
template <typename T>
static std::string Def::printId(const T object)
{
	return '#' + std::to_string(object);
}
