	...
	const uint16_t baseIndent) const
{
	...
	if (not printInline)
		dump.header(Def::Types::Data, UnitizedEstimator<OutU, InUs...>::getUnitSpecifier(prettify), EstimatorBase::getDefIdentifier());
	else
		dump.header("", UnitizedEstimator<OutU, InUs...>::getUnitSpecifier(prettify), "");
