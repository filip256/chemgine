...
def build(sln_file: str, args: list = []) -> bool:
    ...
    try:
        result = subprocess.run(
            [msbuild_path] + [sln_file] + defaults + args, shell=True
        )
...
def build_chem_debug86() -> bool:
    success = build("../Chemgine.sln", ["/p:Configuration=Debug", "/p:Platform=x86"])
...
def build_chem_debug64() -> bool:
    success = build("../Chemgine.sln", ["/p:Configuration=Debug", "/p:Platform=x64"])
...
def build_chem_release86() -> bool:
    success = build("../Chemgine.sln", ["/p:Configuration=Release", "/p:Platform=x86"])
...
def build_chem_release64() -> bool:
    success = build("../Chemgine.sln", ["/p:Configuration=Release", "/p:Platform=x64"])
