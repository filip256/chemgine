...
public:
    ...
    parse(const def::Object& definition, EstimatorRepository& repository)
    {
        ...
        if (const auto parameters =
                definition.getOptionalProperty(def::Data::Parameters, def::parse<std::vector<float_s>>)) {
            ...
            if constexpr (inputCount == 1) {
                if (mode == EstimationMode::LINEAR) {
                    ...
                    return factory.createLinearRegression<OutU, InUs...>(parameters->front(), parameters->back());
                }
            }
