...
public:
    ...
    parse(const def::Object& definition, EstimatorRepository& repository)
    {
        ...
        if (definition.getType() != DefinitionType::DATA &&
            definition.getType() != DefinitionType::AUTO) {
            log.error(
        ...
        if (const auto constant =
                definition.getOptionalProperty(def::Data::Constant, def::parse<Amount<OutU>>)) {
            return factory.createConstant<OutU, InUs...>(*constant);
        }

        if (const auto strValues = definition.getOptionalProperty(
                def::Data::Values, def::parse<std::vector<std::string>>)) {
            const auto mode = definition.getDefaultProperty(
                def::Data::Mode, EstimationMode::LINEAR, def::parse<EstimationMode>);
            const auto loss = definition.getDefaultProperty(
                def::Data::CompressionLoss, 0.0f, def::parse<float_s>);
        ...
        if (const auto parameters = definition.getOptionalProperty(
                def::Data::Parameters, def::parse<std::vector<float_s>>)) {
            ...
            if constexpr (inputCount == 1) {
                if (mode == EstimationMode::LINEAR) {
                    if (parameters->size() != 2) {
                        log.error(
                            "Invalid number of parameters ({0}) for 1st degree linear regression.",
                            parameters->size());
                        return std::nullopt;
                    }
                ...
                if (mode == EstimationMode::LINEAR) {
                    if (parameters->size() != 3) {
                        log.error(
                            "Invalid number of parameters ({0}) for 2nd degree linear regression.",
                            parameters->size());
                        return std::nullopt;
                    }
            ...
            log.fatal(
                "Unsupported regression estimator definition, at: {0}",
                definition.getLocationName());
        }
        ...
        if constexpr (inputCount == 1) {
            ...
            const auto base = definition.getOptionalDefinition(
                def::Data::Base, def::Parser<UnitizedEstimator<OutU, InU>>::parse, repository);
            if (base) {
                // anchor transform
                if (const auto anchorPointStr =
                        definition.getOptionalProperty(def::Data::AnchorPoint)) {
                    const auto anchorPoint = def::parse<DataPoint<OutU, InU>>(
                        *anchorPointStr, OutU, std::vector<Unit>{InU}, definition.getLocation());
                    if (not anchorPoint)
                        return std::nullopt;

                    const auto hShift = definition.getDefaultProperty(
                        def::Data::HorizontalShift, 0.0f, def::parse<float_s>);
                    return factory.createAffine(*base, *anchorPoint, hShift);
                }

                // rebase transform
                if (const auto rebasePointStr =
                        definition.getOptionalProperty(def::Data::RebasePoint)) {
                    const auto rebasePoint = def::parse<DataPoint<OutU, InU>>(
                        *rebasePointStr, OutU, std::vector<Unit>{InU}, definition.getLocation());
                    if (not rebasePoint)
                        return std::nullopt;
                ...
                // manual transform
                const auto vShift = definition.getDefaultProperty(
                    def::Data::VerticalShift, 0.0f, def::parse<float_s>);
                const auto hShift = definition.getDefaultProperty(
                    def::Data::HorizontalShift, 0.0f, def::parse<float_s>);
                const auto scale =
                    definition.getDefaultProperty(def::Data::Scale, 1.0f, def::parse<float_s>);
