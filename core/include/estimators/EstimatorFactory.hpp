...
public:
    ...
    createAffine(const EstimatorRef<OutU, InU>& base, const DataPoint<OutU, InU> rebasePoint);
...
EstimatorRef<OutU, InUs...> EstimatorFactory::createData(
    ...
    const float_s                           maxCompressionLoss)
{
    const auto uniquePoints =
        ImmutableSet<DataPoint<OutU, InUs...>>::toSortedSetVector(std::move(dataPoints));
    ...
    if constexpr (inputCount == 1) {
        ...
        std::transform(
            ...
            [](const DataPoint<OutU, InUs...>& p) {
            return std::tuple(
                std::get<0>(p.inputs).asStd(), std::get<1>(p.inputs).asStd(), p.output.asStd());
        });
    ...
    const float_s                  scale)
{
    ...
    if (utils::floatEqual(vShift, 0.0f) &&
        utils::floatEqual(hShift, 0.0f) &&
        utils::floatEqual(scale, 1.0f))
    ...
    // linear folding
    if (const auto linearBase =
            base.template cast<RegressionEstimator<LinearRegressor2D, OutU, InU>>()) {
    ...
    if (const auto affineBase = base.template cast<AffineEstimator<OutU, InU>>()) {
        return repository.add<AffineEstimator<OutU, InU>>(
            affineBase->getBase(),
            /* vShift= */ -(affineBase->hShift + hShift) * affineBase->scale * scale +
                vShift * (scale + 1.0f),
            /* hShift= */ 0.0f,
...
template <Unit OutU, Unit InU>
EstimatorRef<OutU, InU> EstimatorFactory::createAffine(
    const EstimatorRef<OutU, InU>& base, const DataPoint<OutU, InU> rebasePoint)
{
...
template <Unit OutU, Unit InU>
EstimatorRef<OutU, InU>
EstimatorFactory::createLinearRegression(const float_s paramX, const float_s shift)
{
