...
public:
    ...
    template <
        typename OtherT,
        typename OtherLenT = utils::float_or_unsigned_t<OtherT>,
...
{
    return other.origin.y == this->origin.y &&
           other.origin.x < this->origin.x + this->length &&
           this->origin.x < other.origin.x + other.length;
}
...
{
    return Point(std::min(static_cast<T>(endX()), std::max(origin.x, point.x)), origin.y);
}
...
public:
    ...
    static constexpr bool isNPos(const ObjT& line) { return utils::isNPos(line.origin); }
};
