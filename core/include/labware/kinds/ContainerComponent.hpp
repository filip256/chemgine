...
inline ContainerComponent<Atmosphere>::ContainerComponent(
    const LabwareId id, const LabwareType type, Atmosphere& atmosphere) noexcept :
    BaseContainerComponent(id, type),
    fills(getData().generateShapeFills()),
...
{
    std::for_each(fills.begin(), fills.end(), [&position](auto& f) { f.setPosition(position); });
...
{
    std::for_each(fills.begin(), fills.end(), [&offset](auto& f) { f.move(offset); });
...
{
    std::for_each(fills.begin(), fills.end(), [angle](auto& f) { f.setRotation(angle); });
