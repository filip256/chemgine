...
namespace utils
{
	...
	public:
		constexpr LinearQuantization(
			const InT inLow, const InT inHigh,
			const OutT outLow, const OutT outHigh
		) noexcept;
...
T utils::isqrt(const T x)
{
	...
	// std::sqrt() is likely to be the fastest, but there are quite a few efficient approximative methods:
	...
	res = (res + x / res) >> 1; // x >= 9
    res = (res + x / res) >> 1; // x >= 48
	if constexpr(sizeof(T) >= 2)
		res = (res + x / res) >> 1; // x >= 3712
	if constexpr (sizeof(T) >= 4)
	{
		res = (res + x / res) >> 1; // x >= 16184476
		res = (res + x / res) >> 1; // x >= 4294966935
	}
	if constexpr (sizeof(T) >= 8)
		res = (res + x / res) >> 1; // x >= 18446744073709512015
...
{
	// Output value is computed in high-precision float and clamped to avoid overflow when casted to OutT.
	const auto outValue = std::clamp(value * scale + shift,
