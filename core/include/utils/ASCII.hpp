...
private:
    ...
    constexpr Direction(const uint8_t idx = utils::npos<uint8_t>) noexcept :
        idx(idx)
    {}

public:
    Direction(const int8_t x, const int8_t y) noexcept;
    template <typename T>
...
template <typename T>
Direction::Direction(const Point<T>& point) noexcept :
...
constexpr uint8_t Direction::getIdx() const { return idx; }

constexpr Point<int8_t> Direction::get() const { return _allDirections[idx]; }

constexpr bool Direction::isDiagonal() const { return idx % 2 == 0; }
...
constexpr bool Direction::operator==(const Direction other) const { return this->idx == other.idx; }
...
public:
    ...
    static bool isNPos(const ObjT& direction) { return utils::isNPos(direction.idx); }
};
