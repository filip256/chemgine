...
std::vector<ObjT> flatten(const std::vector<std::pair<ObjT, CntT>>& vector);
...
public:
    static constexpr T value = std::conditional_t<
        std::is_unsigned_v<T>,
        std::integral_constant<T, static_cast<T>(-1)>,
        std::integral_constant<T, std::numeric_limits<T>::lowest()>>::value;
...
template <typename T1, typename T2>
std::pair<T2, T1> utils::reversePair(const std::pair<T1, T2>& pair)
{
...
template <typename KeyT, typename ObjT>
std::unordered_map<ObjT, KeyT> utils::reverseMap(const std::unordered_map<KeyT, ObjT>& map)
{
...
template <typename KeyT, typename ObjT>
std::vector<ObjT> utils::extractValues(const std::unordered_map<KeyT, ObjT>& map)
{
...
template <typename KeyT, typename ObjT>
std::unordered_set<ObjT> utils::extractUniqueValues(const std::unordered_map<KeyT, ObjT>& map)
{
...
template <typename T1, typename T2>
std::vector<T1> utils::extractFirst(const std::vector<std::pair<T1, T2>>& vector)
{
...
template <typename T1, typename T2>
std::vector<T2> utils::extractSecond(const std::vector<std::pair<T1, T2>>& vector)
{
