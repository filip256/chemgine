...
Condenser::Condenser(const LabwareId id, Atmosphere& atmosphere) noexcept :
    ContainerComponent(id, LabwareType::CONDENSER, atmosphere)
{
    getContent<1>().add(Molecule("O"), 10.0_mol);
}

const CondenserData& Condenser::getData() const { return static_cast<const CondenserData&>(data); }
