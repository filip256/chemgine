...
bool PortIdentifier::isValid() const { return componentIdx != LabwareSystem::npos; }

l_size PortIdentifier::getComponentIndex() const { return componentIdx; }
...
const LabwareSystem& PortIdentifier::getSystem() const { return system; }
...
{
    return box.position.x <= boundingBox.position.x ||
           box.position.y <= boundingBox.position.y ||
...
void LabwareSystem::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
     ...
     * bBox.setFillColor(sf::Color(255, 255,
     * 255, 25));
...
LabwareSystem::findClosestPort(const sf::Vector2f& point, const float_s maxSqDistance)
{
    ...
    if (utils::squaredDistance(
            ...
            boundingBox.position.y + boundingBox.size.y) > maxSqDistance)
        return result;
...
std::vector<LabwareSystem> LabwareSystem::disconnect(const l_size componentIdx)
{
    ...
    if (components.size() > 2) {
        ...
        // components and connections are moved away but cannot be erased until the end because of
        // indexes
