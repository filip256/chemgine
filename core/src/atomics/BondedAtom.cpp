...
const Bond* BondedAtomBase::getBondTo(const BondedAtomBase& other) const
{
    ...
    const auto bondIt =
        std::ranges::find_if(bonds, [&](const auto& b) { return other.isSame(b.getOther()); });
...
BondedAtomBase::create(const Symbol& symbol, const c_size index, std::vector<Bond>&& bonds)
{
    return Radical::isDefined(symbol)
               ? static_cast<std::unique_ptr<BondedAtomBase>>(std::make_unique<BondedAtom<Radical>>(
                     Radical(symbol), index, std::move(bonds)))
               : static_cast<std::unique_ptr<BondedAtomBase>>(
                     std::make_unique<BondedAtom<Atom>>(Atom(symbol), index, std::move(bonds)));
}

std::unique_ptr<BondedAtomBase>
BondedAtomBase::create(const Atom& atom, const c_size index, std::vector<Bond>&& bonds)
{
