...
const BondedAtomBase& Bond::getOther() const { return *other; }
...
uint8_t Bond::getValence() const { return Bond::getValence(type); }

std::string Bond::getSMILES() const { return getSMILES(type); }
...
BondType Bond::fromASCII(const char symbol)
{
    ...
    // Extended symbols might have the same value as ASCII symbols (if CHG_EXTENDED_CHAR_SET_ASCII
    // is defined). A separate switch is needed to prevent case collision errors.
...
{
    // Only single bond types can be represented with regular ASCII characters in all the 8
    // directions.
    switch (type) {
    case BondType::IONIC:
    case BondType::SINGLE:
