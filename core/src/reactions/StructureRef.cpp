...
StructureRef::matchWith(const MolecularStructure& structure) const
{
...
bool StructureRef::matchesWith(const StructureRef& other) const
{
    return this->matchWith(other).size();
}

bool StructureRef::operator==(const StructureRef& other) const
{
...
std::optional<StructureRef> StructureRef::create(MolecularStructure&& structure)
{
    return not structure.isEmpty()
               ? std::optional(
                     StructureRef(dataAccessor.get().molecules.findOrAdd(std::move(structure))))
               : std::nullopt;
}
