...
bool SystemMatrix<T>::toREF()
{
    ...
    for (size_t i = 0; i < rows - 1; ++i) {
        ...
        if (std::abs(pivot) < 1e-10) {
            // Duplicate rows may result more than one null rows at the end, thus a null pivot in
            // the penultimate row The system might still be solvable if rows > cols
    ...
    if (cols <= rows) {
        if (std::abs(matrix.back().back()) <
            1e-5)  // 1e-5 could introduce errors in the solution but it allows solving more systems
        {
            matrix.pop_back();
            return true;
        }
