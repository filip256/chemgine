...
{
	return
		line.backwardData().find_first_not_of(WhiteSpace) == ContainerT::npos &&
...
template<typename ContainerT>
std::pair<typename Buffer2D<ContainerT>::IndexT, typename Buffer2D<ContainerT>::IndexT> Buffer2D<ContainerT>::getNonWhiteSpaceSpan(const IndexT idx) const
{
