...
LinearRegressor2D::LinearRegressor2D(const float_s paramX, const float_s shift) noexcept :
    paramX(paramX),
    shift(shift)
{}

float_s LinearRegressor2D::get(const float_s input) const { return input * paramX + shift; }

std::vector<float_s> LinearRegressor2D::getParams() const { return {paramX, shift}; }
