...
LogFormat::LogFormat(const char* format, std::source_location&& location) noexcept :
    format(format),
    location(std::move(location))
{}

LogFormat::LogFormat(const std::string& format, std::source_location&& location) noexcept :
    LogFormat(format.c_str(), std::move(location))
{}

const char* LogFormat::getFormat() const { return format; }

const std::source_location& LogFormat::getLocation() const { return location; }

uint8_t              LogBase::contexts  = 0;
size_t               LogBase::foldCount = static_cast<size_t>(-1);
std::vector<LogType> LogBase::hideStack = {};

LogBase::LogBase(const void* address, const std::type_index sourceType) noexcept :
    address(address),
