...
Direction Direction::turn(const Angle angle, const bool parallelIsBackward) const
{
    ...
    if constexpr (R == Rotation::CLOCKWISE)
        ...
        return Direction(static_cast<uint8_t>((idx - step + Direction::AllDirections.size()) % Direction::AllDirections.size()));
}
...
namespace
{
    constexpr uint8_t getAngleIndex(const Direction d1, const Direction d2)
    {
        ...
        //  1-3 = CCW
        //  5-7 = CW
        return static_cast<uint8_t>((d1.getIdx() - d2.getIdx() + Direction::AllDirections.size()) % Direction::AllDirections.size());
    }
}
