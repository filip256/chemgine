...
static uint8_t getPixel(const TextureMask& mask, const sf::Texture& tex, uint32_t x, uint32_t y)
{
...
bool singlePixelTest(const sf::Sprite& sprite, const sf::Vector2f& mousePosition, const uint8_t alphaLimit)
{
    ...
    auto sv = sprite.getInverseTransform().transformPoint(mousePosition);
    // Make sure pixels fall within the sprite's subrect
    if (sv.x > 0 && sv.y > 0 && sv.x < subRect.size.x && sv.y < subRect.size.y) {
        return getPixel(
                   mask,
                   sprite.getTexture(),
                   static_cast<int>(sv.x) + subRect.position.x,
                   static_cast<int>(sv.y) + subRect.position.y) > alphaLimit;
    }
    return false;
}
    ...
    for (auto i = intersection->position.x; i < intersection->position.x + intersection->size.x; ++i) {
        for (auto j = intersection->position.y; j < intersection->position.y + intersection->size.y; ++j) {
            ...
            if (s1v.x > 0 &&
                ...
                s2v.x > 0 &&
                s2v.y > 0 &&
                s1v.x < s1SubRect.size.x &&
                s1v.y < s1SubRect.size.y &&
                s2v.x < s2SubRect.size.x &&
                s2v.y < s2SubRect.size.y) {
                if (getPixel(
                        mask1,
                        sprite1.getTexture(),
                        (int) (s1v.x) + s1SubRect.position.x,
                        (int) (s1v.y) + s1SubRect.position.y) > alphaLimit &&
                    getPixel(
                        mask2,
                        sprite2.getTexture(),
                        (int) (s2v.x) + s2SubRect.position.x,
                        (int) (s2v.y) + s2SubRect.position.y) > alphaLimit)
                    return true;
            }
        }
    }
...
SizedTextureMask createBitmask(const std::string& filename)
{
    ...
    if (not img.loadFromFile(filename))
        Log<>().fatal("Failed to load texture from file: {}.", filename);
...
struct OrientedBoundingBox  // Used in the BoundingBoxTest
{
    ...
    OrientedBoundingBox(const sf::Sprite& sprite)  // Calculate the four points of the OBB from a
    ...
    // Project all four points of the OBB onto the given axis and return the dot products of the two
    // outermost points
...
const TextureMask& getTextureMask(const sf::Texture& tex) { return bitmasks().get(tex); }
