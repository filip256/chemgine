...
namespace Collision
{
		...
		for (auto i = intersection->position.x; i < intersection->position.x + intersection->size.x; ++i) {
			for (auto j = intersection->position.y; j < intersection->position.y + intersection->size.y; ++j) {
				...
				if (s1v.x > 0 && s1v.y > 0 && s2v.x > 0 && s2v.y > 0 &&
					...
					if (getPixel(mask1, sprite1.getTexture(), (int)(s1v.x) + s1SubRect.position.x, (int)(s1v.y) + s1SubRect.position.y) > alphaLimit &&
						getPixel(mask2, sprite2.getTexture(), (int)(s2v.x) + s2SubRect.position.x, (int)(s2v.y) + s2SubRect.position.y) > alphaLimit)
						return true;
	...
	struct OrientedBoundingBox // Used in the BoundingBoxTest
	{
		...
		OrientedBoundingBox(const sf::Sprite& sprite) // Calculate the four points of the OBB from a transformed (scaled, rotated...) sprite
		{
			...
			points[2] = transform.transformPoint(sf::Vector2f(static_cast<float>(local.size.x), static_cast<float>(local.size.y)));
