...
void ParticleSystem::setColor(const sf::Color& color) { this->color = color; }

void ParticleSystem::setSpeed(const Amount<Unit::PER_SECOND> speed) { this->speed = speed; }

void ParticleSystem::setLifespan(const Amount<Unit::SECOND> lifespan) { this->lifespan = lifespan; }
...
{
    for (uint16_t i = 0; i < particles.size(); ++i) particles[i].tick(timespan);
}

void ParticleSystem::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
    for (uint16_t i = 0; i < particles.size(); ++i) target.draw(particles[i], states);
}
