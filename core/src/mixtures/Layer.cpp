...
Amount<Unit::JOULE> Layer::getLeastEnergyDiff(const Amount<Unit::CELSIUS> target) const
{
    ...
    return getTotalHeatCapacity().to<Unit::JOULE>(std::max(target, transitionPoint) - temperature);
}
...
void Layer::convertTemporaryStateReactants()
{
    ...
    //        - temp reactats with smaller diffs act towards reaching the tp of those with higher or
    //        equal diffs
...
LayerType Layer::getType() const { return layerType; }

Amount<Unit::MOLE> Layer::getMoles() const { return moles; }

Amount<Unit::GRAM> Layer::getMass() const { return mass; }

Amount<Unit::LITER> Layer::getVolume() const { return volume; }

bool Layer::hasLowNucleator() const { return lowNucleator.isValid(); }

bool Layer::hasHighNucleator() const { return highNucleator.isValid(); }

const Reactant& Layer::getLowNucleator() const { return lowNucleator.getReactant(); }

const Reactant& Layer::getHighNucleator() const { return highNucleator.getReactant(); }

Amount<Unit::CELSIUS> Layer::getTemperature() const { return temperature; }
...
Polarity Layer::getPolarity() const { return polarity; }
...
bool Layer::isEmpty() const { return moles == 0.0; }
