...
// Fuzzing works by generating distinct SMILES notations for each input and parsing them back into molecules.
// The order of the original inputs is preserved for the fuzzed inputs.
// The test is also applied on the original inputs.
...
bool ASCIIPrintUnitTest::run()
{
	...
	{
		Log(this).error("Equality check between printed molecule: '{0}' and input: '{1}' failed:\n{2}",
			parsedMolecule->toSMILES(), molecule.toSMILES(), ascii);
...
bool MolBinUnitTest::run()
{
	...
	if (*parsedMolecule != molecule)
	{
		Log(this).error("Equality check between printed molecule: '{}' and input: '{}' failed.",
			parsedMolecule->toSMILES(), molecule.toSMILES());
		return false;
	}
	...
	UnitTestGroup(std::move(name), filter)
{
	...
	registerTest<StructureSMILESUnitTest>("SMILES", "C3=CC27CC18C=CC16C=C%10CCC%12C%11C=C5C=C4C(C=C2C3)C49C5=C(C6C789)C%10%11%12", 356.47f);
	...
	registerTest<StructureEqualityUnitTest>("equality", "C1C2C3C4C1C15C6CC7C8C6C6CC8C8(C3CC4C618)C275", "C1C2C3C4CC5C3C1C67C28C49C56C1CC9C2C8CC7C12", true);
	registerTest<StructureEqualityUnitTest>("equality", "[Cl]", "X", false);
	registerTest<StructureEqualityUnitTest>("equality", "N2C1=CC=C(OC)C=C1C(CCN(C)C)C2", "N2C1=CC=C(OC)C=C1C(C2)CCN(C)C", true);
	...
	registerTest<MinimalCycleUnitTest>("minimal_cycle", "C12C3C1C23", 4, std::unordered_map<c_size, c_size>({ { 3, 3 } }));
	registerTest<MinimalCycleUnitTest>("minimal_cycle", "C12C3C4C1C234", 5, std::unordered_map<c_size, c_size>({ { 3, 4 } }));
	registerTest<MinimalCycleUnitTest>("minimal_cycle", "C12C3C4C1C5C2C3C45", 8, std::unordered_map<c_size, c_size>({ { 4, 5 } }));
	...
	registerTest<ASCIIPrintUnitTest>("ASCII", "[Si][Si]N([Si][Si])[Si](=O)[Si]1[Si]N([Si]2[Si][Si]3=[Si]N[Si]4=[Si][Si]=[Si][Si](=[Si]34)[Si]2=[Si]1)[Si]", false);
	registerTest<ASCIIPrintUnitTest>("ASCII", "CC(=O)OC1=C2OC4C(OC(C)=O)C=CC3C5CC(C=C1)=C2C34CCN5C", true);
