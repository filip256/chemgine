...
LinearSplinePerfTest::LinearSplinePerfTest(
    ...
    const uint32_t                                       size,
    const float_s                                        loss) noexcept :
    TimedTest(std::move(name), limit),
    loss(loss),
    input(generateInput(generator, size))
{}

std::vector<std::pair<float_s, float_s>>
LinearSplinePerfTest::generateInput(float_s (*generator)(const float_s), const uint32_t size)
{
...
EstimatorPerfTests::EstimatorPerfTests(std::string&& name, const std::regex& filter) noexcept :
    PerfTestGroup(std::move(name), filter)
{
    registerTest<LinearSplinePerfTest>(
        "lspline_quadratic", std::chrono::seconds(5), [](const float_s x) -> float_s {
        return x * x;
    }, 10'000, 0.0f);
    registerTest<LinearSplinePerfTest>(
    ...
    registerTest<LinearSplinePerfTest>(
        "lspline_sin", std::chrono::seconds(5), [](const float_s x) -> float_s {
        return std::sin(x);
    }, 10'000, 0.0f);
    registerTest<LinearSplinePerfTest>(
        "lspline_sin_loss", std::chrono::seconds(5), [](const float_s x) -> float_s {
        return std::sin(x);
    }, 10'000, 0.5f);
}
