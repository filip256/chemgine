...
public:
    PerfTestSetup(std::string&& name, SetupT&& setup) noexcept;
...
template <typename SetupT>
PerfTestSetup<SetupT>::PerfTestSetup(std::string&& name, SetupT&& setup) noexcept :
...
private:
    ...
    static OS::ExecutionConfig stabilizeExecution();
    static void                restoreExecution(const OS::ExecutionConfig normalProperties);
...
public:
    PerfTestGroup(std::string&& name, const std::regex& filter) noexcept;
