...
public:
    UnitTestSetup(std::string&& name, SetupT&& setup) noexcept;
...
template <typename SetupT>
UnitTestSetup<SetupT>::UnitTestSetup(std::string&& name, SetupT&& setup) noexcept :
...
public:
    UnitTestGroup(std::string&& name, const std::regex& filter) noexcept;
