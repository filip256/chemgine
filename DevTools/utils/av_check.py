...
vt_api_key = "1c85e604996cfd5b41c47f50a8a8d3c1022b857783524dda224cf64d661aec5d"
...
async def vt_check_dir(path :str):
  ...
  for _ in range(min(4, n_files)):
    worker_tasks.append(asyncio.create_task(upload_hashes(queue, vt_api_key)))

  analyses = itertools.chain.from_iterable(await asyncio.gather(*worker_tasks))
  await asyncio.gather(
      *[
          asyncio.create_task(process_analysis_results(vt_api_key, a, f))
          for a, f in analyses
      ]
  )


async def vt_check_files(files :list):
  queue = asyncio.Queue()
  for f in files:
        await queue.put(f)
